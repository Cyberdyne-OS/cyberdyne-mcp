...
const found = await call("search_humans", { skill: "audio", language: "es" });
...
const posted = await call("post_task", {
  description: "Read 10 phrases", category: "audio", criteria: "clear audio, quiet room", reward: 3.5
});
...
const rejectMatch = await call("search_humans", { skill: "observation", device: "car" });
console.log(`search_humans(observation+car) → ${rejectMatch.count} match:`, rejectMatch.humans.map((h: any) => h.handle).join(", "));
