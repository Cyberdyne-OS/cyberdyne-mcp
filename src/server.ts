...
if (process.argv[2] === "onboard") {
  try {
    ...
    console.error(
      `✓ CYBERDYNE agent onboarded — wallet ${mode} + API key minted.\n` +
...
server.tool(
  ...
  async ({ task_id }) =>
    guard(async () => {
      ...
      if (!info) {
        throw new Error(
          "this task has no escrow_payment_info — it was never frozen on the non-custodial pool escrow, so there is nothing to reclaim on-chain.",
        );
      }
      const { hasEvmKey, reclaimBudget } = await import("./evm-signer.js");
      if (!hasEvmKey()) {
        throw new Error(
          "no signing wallet — reclaim is an on-chain call you make yourself. Set CYBERDYNE_EVM_PRIVATE_KEY or run `npx cyberdyne-mcp onboard` with the SAME wallet that froze the budget.",
        );
      }
