 ...
 */
export function privateKeyFromSecret(secret: string): `0x${string}` {
  const s = secret.trim();
  if (isHexPrivateKey(s)) return normalizeKey(s);
  if (looksLikeMnemonic(s)) {
    ...
    if (!pk) throw new Error("could not derive a private key from that mnemonic");
    return bytesToHex(pk);
  }
  throw new Error(
    "malformed wallet secret — expected a 0x-prefixed 64-hex-char private key, or a valid BIP-39 mnemonic (12–24 words).",
  );
}
...
export interface ResolvedWallet {
  ...
  generated: boolean;
  /** true when the caller explicitly imported a key/mnemonic this call. */
  imported: boolean;
}
...
export function resolveWallet(
  ...
  opts: { importSecret?: string; forceCreate?: boolean } = {},
): ResolvedWallet {
...
export interface OnboardOptions {
  /** A user-supplied hex private key or BIP-39 mnemonic to IMPORT as the agent wallet. */
  importSecret?: string;
...
export async function onboardCli(argv: string[], env: NodeJS.ProcessEnv = process.env): Promise<OnboardResult> {
  ...
  if (importFlag) {
    ...
    if (!secret) {
      throw new Error(
        "--import needs a key/mnemonic. Pipe it (echo 0x<key> | npx cyberdyne-mcp onboard --import), " +
    ...
    const answer = (
      await promptLine("Import an existing wallet (paste private key / mnemonic) or press enter to create a new one: ")
    ).trim();
  ...
  if (fromArgv) {
    console.error(
      "⚠  Heads-up: passing the wallet secret as an argument leaves it in your shell history.\n" +
