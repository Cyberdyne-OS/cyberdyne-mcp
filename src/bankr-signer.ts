...
export function hasBankrSigner(): boolean {
 ...
 */
export async function bankrAccount(key?: string) {
  const bk = key ?? resolveBankrKey();
  if (!bk) throw new MissingBankrKeyError();
...
export async function bankrSignerAddress(key?: string): Promise<string> {
 ...
 */
async function assertPermit2Ready(requirements: unknown, owner: string): Promise<void> {
  const r = (requirements ?? {}) as { asset?: string; amount?: string | number; extra?: { assetTransferMethod?: string } };
  if (r.extra?.assetTransferMethod !== "permit2") return; // USDC/EIP-3009: nothing to approve
  const token = r.asset;
  ...
  if (need === 0n) return; // nothing to freeze → no approval needed
  ...
  throw new Error(
    `Bankr-wallet funding for this Permit2 token (${token}) needs a one-time ERC-20→Permit2 approval ` +
 ...
 */
export async function bankrSignAuthCapture(requirements: unknown, key?: string): Promise<string> {
  const bk = key ?? resolveBankrKey();
  if (!bk) throw new MissingBankrKeyError();
 ...
 */
export async function bankrPayDeployFee(
  fee: { amount: number | string; recipient: string; token: string },
  key?: string,
): Promise<string> {
  ...
  const hash = await bankrTransfer(
    { tokenAddress: getAddress(fee.token), recipientAddress: getAddress(fee.recipient), amount: fee.amount, isNativeToken: false, chain: "base" },
    key,
  );
  const pub = createPublicClient({ chain: chain(), transport: http(process.env.CYBERDYNE_RPC_URL) });
  const receipt = await pub.waitForTransactionReceipt({ hash: hash as `0x${string}`, confirmations: 2 });
