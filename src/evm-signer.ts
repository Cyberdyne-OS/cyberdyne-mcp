 ...
 */
async function ensurePermit2Approval(requirements: unknown): Promise<void> {
  const r = (requirements ?? {}) as { asset?: string; amount?: string | number; extra?: { assetTransferMethod?: string } };
  if (r.extra?.assetTransferMethod !== "permit2") return; // EIP-3009 / USDC: nothing to approve
  const token = r.asset;
  if (!token || !isAddress(token)) return;
  const owner = getAddress(evmAddress());
  const need = (() => { try { return BigInt(r.amount ?? 0); } catch { return 0n; } })();
  const pub = createPublicClient({ chain: chain(), transport: http(process.env.CYBERDYNE_RPC_URL) });
  ...
  if (receipt.status !== "success") throw new Error(`Permit2 approval tx ${hash} reverted — budget cannot be frozen for ${token}`);
}
...
export const AUTH_CAPTURE_ESCROW_ADDRESS = "0xBdEA0D1bcC5966192B070Fdf62aB4EF5b4420cff" as const;
...
const paymentInfoArg = { name: "paymentInfo", type: "tuple", components: PAYMENT_INFO_COMPONENTS } as const;
...
/** The stored PaymentInfo jsonb (EscrowPaymentInfo) as returned by GET /api/tasks/[id]. */
...
function decodeSignedPayment(signedPayment: string): { payer: string; preApprovalExpiry: number; salt: string; value?: string } {
  let payload: Record<string, unknown>;
  try {
    const decoded = JSON.parse(Buffer.from(signedPayment, "base64").toString("utf8"));
    payload = (decoded.payload ?? decoded) as Record<string, unknown>;
  } catch {
    throw new Error("malformed_signed_payment");
  }
  const auth = payload.authorization as { from?: string; validBefore?: string; value?: string } | undefined;
  const permit = payload.permit2Authorization as { from?: string; deadline?: string } | undefined;
  ...
  if (!from || !validBefore || !salt) throw new Error("incomplete_signed_payment (need authorization.from + validBefore + salt)");
 ...
 */
function structFor(info: StoredPaymentInfo, payer: string, preApprovalExpiry: number, salt: string, value?: string) {
  const x = info.extra;
  return {
 ...
 */
export async function reclaimBudget(info: StoredPaymentInfo): Promise<{ ok: true; tx_hash: string; reclaimed: string }> {
  const signed = info.signedPayment;
  if (!signed) throw new Error("no signedPayment on this task — it was never frozen on the escrow (nothing to reclaim)");
  ...
  if (me.toLowerCase() !== payer.toLowerCase()) {
    throw new Error(
      `wallet ${me} is not the payer (${payer}) of this budget — reclaim is payer-only on-chain. ` +
  ...
  if (now < paymentInfo.authorizationExpiry) {
    ...
    throw new Error(
      `too_early: the authorization deadline has not passed yet (reclaim opens at ${when}, ` +
  ...
  if (receipt.status !== "success") {
    throw new Error(
      "reclaim reverted on-chain — the budget may already be fully captured/settled, already reclaimed, " +
